...
namespace myStructs {
	...
	struct Mesh {
		...
		void AddVertex(Vector3 v, Vector3 color) {
			...
			//int index = -1;
			//if (existingVerticies.find(v.toString()) == existingVerticies.end()) {
			...
			//	existingVerticies.insert({ v.toString(), (unsigned int)index });
			//}
			//
			//triangles.push_back(index == -1 ? existingVerticies[v.toString()] : index);
		}
